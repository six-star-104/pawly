...
export const getEasterEggs = async (): Promise<EasterEggResponse> => {
  try {
...
// 도전과제 완료 요청 함수
export const completeEasterEgg = async (easterEggId: number): Promise<EasterEggResponse> => {
  try {
    const response = await axiosInstance.post<EasterEggResponse>('/easter-egg', { easterEggId });
