...
import {
  ...
  MemberInfoResponse,
} from "@/types/FriendsTypes"; // 타입 import
...
export const getFriendRequestsReceived = async () => {
  try {
    const response = await axiosInstance.get<FriendRequestResponse>(
      "friend/response"
    );
...
export const respondToFriendRequest = async (
  ...
  try {
    ...
    const response = await axiosInstance.patch<FriendResponse>(
      "/friend",
      requestData
    );
    return response.data;
...
export const getFriendRequestsSent = async () => {
  try {
    const response = await axiosInstance.get<SentFriendRequestResponse>(
      "/friend/request"
    );
    ...
    // if (error.response) {
    //   console.error(
    //     "서버 오류로 인해 친구 신청한 목록 조회 실패:",
    //     error.response.data.message
    //   );
    //   throw new Error(
    //     error.response.data.message ||
    //       "친구 신청한 목록 조회 중 오류가 발생했습니다."
    //   );
    // } else if (error.request) {
    //   console.error(
    //     "네트워크 오류로 인해 친구 신청한 목록 조회 실패:",
    //     error.message
    //   );
    //   throw new Error(
...
export const deleteFriend = async (memberId: number) => {
  try {
    const response = await axiosInstance.delete<DeleteFriendResponse>(
      "/friend",
      {
        data: { memberId },
      }
