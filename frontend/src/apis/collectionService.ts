...
// 도감 조회 API 함수
export const getCollection = async (memberId: number, pageNumber: number, pageSize: number): Promise<ICollectionResponse> => {
  try {
    console.log(`API에서 받아오는 데이터= memberId=${memberId}, pageNumber=${pageNumber}, pageSize=${pageSize}`);
    
    const response = await axiosInstance.get<ICollectionResponse>(`/collection/${memberId}`, {
